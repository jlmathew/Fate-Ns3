 ...
 */
#include "ForwardNs3Ipv4ManagerF2.h"

//extern std::map<ns3::Ipv4Address, std::string > dnsIpToName;

ForwardNs3Ipv4ManagerF2::ForwardNs3Ipv4ManagerF2()
  : ForwardNs3Ipv4Manager::ForwardNs3Ipv4Manager(),
    m_dns(nullptr)
{}

ForwardNs3Ipv4ManagerF2::ForwardNs3Ipv4ManagerF2(ConfigWrapper &config)
...
bool
ForwardNs3Ipv4ManagerF2::OnInit(UtilityExternalModule *data)
{
...
bool
ForwardNs3Ipv4ManagerF2::isThisMyIpAddr(const std::string &myName,  ns3::Ipv4Address addr)
{
  assert(m_dns);
...
double
ForwardNs3Ipv4ManagerF2::PopNextDst(PktType &pkt, std::string &nextAddr)
{
  std::string dstAddr;
  std::string srcAddr;
...
void
ForwardNs3Ipv4ManagerF2::OnPktIngress(PktType &pkt) {
  ...
  std::string srcAddr;
  double cacheHit = 0.0;
  bool existHit = pkt.GetNamedAttribute("CacheHit",cacheHit);
  //uint64_t tmp;
