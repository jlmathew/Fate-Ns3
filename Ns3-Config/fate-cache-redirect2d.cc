...
std::ifstream ifs(origFileConfig);
  std::string origConfig( (std::istreambuf_iterator<char>(ifs) ),
	...
	{
		std::string config=origConfig;
		unsigned int first=config.find("MDU");
		if (first == std::string::npos) { assert(0);}
		std::string hashValue=std::to_string(nodes.GetN()*cSize);
		config.replace(config.begin()+first, config.begin()+first+3,hashValue);


		first=config.find("LOW");
		if (first == std::string::npos) { assert(0);}
		hashValue=std::to_string(i*cSize);
		config.replace(config.begin()+first, config.begin()+first+3,hashValue);

		unsigned int first2=config.find("HIGH");
		if (first2 == std::string::npos) { assert(0);}
		std::string hashValue2=std::to_string((i+1)*cSize-1);
		config.replace(config.begin()+first2, config.begin()+first2+4,hashValue2);
		Ptr<Node> node = *it;
		std::cout << "Node " << node->GetId() << " autohash [" << hashValue <<"," << hashValue2 << "]\n";
		//std::cout << "Node " << *it.GetId() << " autohash [" << hashValue <<"," << hashValue2 << "]\n";

		std::cout << "new config is " << config << "\n";
	UtilityConfigXml cconfig;
	//system sed file ? ; TODO FIXME JLM

	std::ofstream out("/tmp/overwritten.xml");
    out << config;
    out.close();

      cconfig.FirstNodeFileConfig("/tmp/overwritten.xml");
      //cconfig.FirstNodeFileConfig(config);
      FateIpv4Helper helper;
      helper.SetConfigFile("/tmp/overwritten.xml");
...
Packet::EnableChecking();
  ...
  //std::string cConfig("");  //no config or name of xml file
   ...
   cConfig="fateXmlConfigFiles/LruHash-reroute3.xml";  //no config or name of xml file
  //std::string nConfig("");
