...
NS_LOG_COMPONENT_DEFINE ("UdpFateCbrClientApplication");

NS_OBJECT_ENSURE_REGISTERED (UdpFateCbrClient);

TypeId
UdpFateCbrClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpFateCbrFateCbrClient")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<UdpFateCbrClient> ()
    .AddAttribute ("MaxPackets", 
                   "The maximum number of packets the application will send",
                   UintegerValue (100),
                   MakeUintegerAccessor (&UdpFateCbrClient::m_count),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Interval", 
                   "The time to wait between packets",
                   TimeValue (Seconds (1.0)),
                   MakeTimeAccessor (&UdpFateCbrClient::m_interval),
                   MakeTimeChecker ())
    .AddAttribute ("RemoteAddress", 
                   "The destination Address of the outbound packets",
                   AddressValue (),
                   MakeAddressAccessor (&UdpFateCbrClient::m_peerAddress),
                   MakeAddressChecker ())
    .AddAttribute ("RemotePort", 
                   "The destination port of the outbound packets",
                   UintegerValue (0),
                   MakeUintegerAccessor (&UdpFateCbrClient::m_peerPort),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("NumSegments", "Number of packet segments which compose a file",
                   UintegerValue (1),
                   MakeUintegerAccessor (&UdpFateCbrClient::m_segSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("NumFiles", "Number of unique files sent",
                   UintegerValue (1),
                   MakeUintegerAccessor (&UdpFateCbrClient::m_maxFiles),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("AddTimeStamp", "Add a timestamp to each packet",
                   BooleanValue (false),
                   MakeBooleanAccessor (&UdpFateCbrClient::m_timestamp),
                   MakeBooleanChecker())
    .AddTraceSource ("Tx", "A new packet is created and is sent",
                     MakeTraceSourceAccessor (&UdpFateCbrClient::m_txTrace),
                     "ns3::Packet::TracedCallback")
  ;
  return tid;
}

UdpFateCbrClient::UdpFateCbrClient ()
{
...
void 
UdpFateCbrClient::SetRemote (Address ip, uint16_t port)
{
...
void 
UdpFateCbrClient::SetRemote (Address addr)
{
...
void
UdpFateCbrClient::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  Application::DoDispose ();
}
void 
UdpFateCbrClient::SetTimestamp(bool timestamp) {
  m_timestamp = timestamp;
}

void 
UdpFateCbrClient::StartApplication (void)
{
  NS_LOG_FUNCTION (this);
  ...
  m_socket->SetRecvCallback (MakeCallback (&UdpFateCbrClient::HandleRead, this));
  m_socket->SetAllowBroadcast (true);
  ScheduleTransmit (Seconds (0.));
}

void 
UdpFateCbrClient::StopApplication ()
{
  NS_LOG_FUNCTION (this);
...
uint32_t 
UdpFateCbrClient::GetDataSize (void) const
{
  NS_LOG_FUNCTION (this);
  return m_size;
}
...
void 
UdpFateCbrClient::SetPktPayload (const std::string &xml)
{
...
void 
UdpFateCbrClient::SetPktPayload (const PktType &payload)
{
  m_payload = payload;
     std::vector<uint8_t> fateData;
   m_payload.Serialize(fateData);
...
void 
UdpFateCbrClient::SetPktPayload (const IcnName<std::string> &name)
{
  m_payload.ClearData();
...
void 
UdpFateCbrClient::SetPktPayload (uint8_t *fill, uint32_t dataSize)
{
...
void 
UdpFateCbrClient::ScheduleTransmit (Time dt)
{
  NS_LOG_FUNCTION (this << dt);
  m_sendEvent = Simulator::Schedule (dt, &UdpFateCbrClient::Send, this);
}

void 
UdpFateCbrClient::Send (void)
{
  NS_LOG_FUNCTION (this);
...
void
UdpFateCbrClient::HandleRead (Ptr<Socket> socket)
{
  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      if (InetSocketAddress::IsMatchingType (from))
        {
          NS_LOG_INFO ("At time " << Simulator::Now () << "/" << timesent << " 's client received " << packet->GetSize () << " bytes from " << InetSocketAddress::ConvertFrom (from).GetIpv4 () << " port " <<
...
void
UdpFateCbrClient::PrintStats(std::ostream &os) const
{
