...
void setProducers(void) {
    ...
    for(unsigned int i=0; i< producers.GetN(); i++)
    {
      ...
      std::string matchName="/test1";//+std::to_string(i+1);
   ...
   unsigned int i=0; 
   {
    ...
    //add application to each
     UdpFateZipfClientHelper echoClient(ipic[0].GetAddress (1),9);   //based upon the name, it will map to an ip/port
       ...
       //echoClient.SetAttribute ("matchByType", StringValue("location")); //filenum, location, segnum
       bool val = false; //(sendOff=="true" ? true : false);
      ...
      //Need to make this variable AND loop for all consumers 
       std::string temp="/test1";//+std::to_string( (i%producers.GetN())+1);
       ...
       //std::string cacheTrip="10.0.0.14;"; //<--redirect cache IP
       //cacheTrip.push_back(';');
...
void createSubCacheFateNodes(const std::string &origFileConfig, const NodeContainer &nodes)
{
...
int i=0;
std::ifstream ifs(origFileConfig);
  std::string origConfig( (std::istreambuf_iterator<char>(ifs) ),
                       (std::istreambuf_iterator<char>()    ) );
  
	for(auto it=nodes.Begin(); it!=nodes.End(); it++)
	{
		Ptr<Node> node = *it;
		//std::cout << "Node " << *it.GetId() << " autohash [" << hashValue <<"," << hashValue2 << "]\n";
        std::string FileConfig="fateXmlConfigFiles/Lru-reroute"+std::to_string(i)+".xml"; 
	UtilityConfigXml cconfig;


      cconfig.FirstNodeFileConfig(FileConfig);
      //cconfig.FirstNodeFileConfig(config);
      FateIpv4Helper helper;
      helper.SetConfigFile(FileConfig);
...
createSubCacheFateNodes(cConfig, caches);
...
{
    if (i ==nCaches || i == (nCaches+1) || i == (nCaches+2) || i == nProd || i==nCons)
     {
         std::cout << "exclude node " << i <<"\n";   continue;
     }
    restNodes.Add(NodeContainer::GetGlobal().Get(i));
}
...
void createProducers(unsigned int n)
{
   ...
   {
       producers.Add(NodeContainer::GetGlobal().Get(n));
   } 
   std::cout << "\n";
   setProducers();
}
void createConsumers(unsigned int n)
{
   ...
   //for(unsigned int i=0; i<n; i++)
   {
       consumers.Add(NodeContainer::GetGlobal().Get(n));
   } 
   std::cout << "\n";
...
void createCaches(unsigned int n)
{
   ...
   //for(unsigned int i=0; i<n; i++)
   {
       caches.Add(NodeContainer::GetGlobal().Get(n));
       caches.Add(NodeContainer::GetGlobal().Get(n+1));
       caches.Add(NodeContainer::GetGlobal().Get(n+2));
   } 
   std::cout << "\n";
}
...
Packet::EnableChecking();
  ...
  input="scratch/hash-3-cache.orb";
  ...
  //std::string cConfig("");  //no config or name of xml file
   ...
   cConfig="fateXmlConfigFiles/Lru-reroute30.xml";  //no config or name of xml file
  //std::string nConfig("");
  nConfig="fateXmlConfigFiles/nocache2.xml";
 ...
 Config::SetDefault ("ns3::PointToPointNetDevice::DataRate", StringValue ("10Gbps"));
  ...
  Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
    ...
    //JLM CreateDestAssociation(NodeContainer::GetGlobal());
    //how to do caching
    /*if (cConfig.size()) {
